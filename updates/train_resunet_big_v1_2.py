"""
train_resunet_big_v1_2.py  —  BigResUNet v1.2 (= v1 + higher dB RSE weight)

v1 used w_mag_db=1.0. v1.2 raises it to 3.0, forcing the model to prioritize
getting dB-scale predictions right over the R/I MSE and smoothness terms.

Hypothesis: a stronger dB-scale gradient pushes the model harder toward
capturing the resonance shape, potentially improving both dB RSE and
magnitude RSE metrics.

Everything else is identical to v1 (BigResUNet1D, no Gaussian smooth,
CosineAnnealingLR, no early stopping, 300 epochs).

COMPARISON TABLE:
  Model   w_mag_db  smooth  best_val
  v1      1.0       ✗       0.0332  ← base
  v1.2    3.0       ✗       ?       ← this file
"""
...
# ---------------------------------------------------------------------------
# Loss  (identical to v1 — only w_mag_db default changes)
# ---------------------------------------------------------------------------
...
def train(args: argparse.Namespace) -> None:
    ...
    print(f"BigResUNet1D (v1.2)  |  base_ch={args.base_channels}  |  trainable params: {n_params:,}")
    print(f"Device: {device}")
    print(f"w_mag_db={args.w_mag_db} (v1 baseline was 1.0)")
    ...
    meta = {
        ...
        "motivation": (
            "v1.2: v1 + higher dB RSE weight (w_mag_db=3.0 vs v1's 1.0). "
            "Stronger dB-scale gradient pushes model harder toward resonance shape accuracy."
        ),
        "loss_weights": {
            "w_ri":        args.w_ri,
            "w_mag_db":    args.w_mag_db,
            "db_loss":     "flat dB RSE (same as v1, weight increased)",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train BigResUNet v1.2 — v1 + w_mag_db=3.0")
    parser.add_argument("--project-root",  type=str,   default=".")
    parser.add_argument("--output-dir",    type=str,   default="NNModel")
    parser.add_argument("--model-name",    type=str,   default="trained_model_resunet_v1_2.pt")
    parser.add_argument("--scaler-name",   type=str,   default="scaler_resunet_v1_2.gz")
    parser.add_argument("--history-name",  type=str,   default="history_resunet_v1_2.csv")
    parser.add_argument("--meta-name",     type=str,   default="meta_resunet_v1_2.json")
    ...
    parser.add_argument("--w-mag-db",      type=float, default=3.00)   # ← 3x v1
