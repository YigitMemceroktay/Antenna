"""
train_resunet_big_v1_1.py  —  BigResUNet v1.1 (= v1 + weighted dB RSE)

v1 used flat dB RSE loss, which treats all frequency points equally.
v1.1 adds per-point weights inversely proportional to magnitude, so that
deep resonance dips (mag_t ≈ 0) receive higher gradient signal.

Weight formula (from DualResUNet):
    w[f] = (1 / clamp(mag_t[f], eps)) ^ alpha
    w[f] = clamp(w[f], max=db_weight_max)
    w[f] /= mean(w)           # normalize to unit mean

Everything else is identical to v1 (BigResUNet1D, no Gaussian smooth,
CosineAnnealingLR, no early stopping, 300 epochs).

COMPARISON TABLE:
  Model   loss            smooth  best_val
  v1      flat dB RSE     ✗       0.0332  ← base
  v1.1    weighted dB RSE ✗       ?       ← this file
"""
...
# ---------------------------------------------------------------------------
# Loss  (weighted dB RSE — NEW vs v1)
# ---------------------------------------------------------------------------
...
def compute_loss(
    ...
    # Weighted dB RSE: deep dips (small mag_t) get higher weight
...
# ---------------------------------------------------------------------------
# Data loading  (old_excel only — LHS is test set)
# ---------------------------------------------------------------------------
...
def train(args: argparse.Namespace) -> None:
    ...
    print(f"BigResUNet1D (v1.1)  |  base_ch={args.base_channels}  |  trainable params: {n_params:,}")
    print(f"Device: {device}")
    print(f"Weighted dB RSE: alpha={args.db_weight_alpha}, max_weight={args.db_weight_max}")
    ...
    meta = {
        ...
        "motivation": (
            "v1.1: v1 + weighted dB RSE loss. "
            "Deep resonance dips (small mag_t) receive higher gradient signal "
            "via per-frequency weights = (1/mag_t)^alpha."
        ),
        "loss_weights": {
            "w_ri":           args.w_ri,
            "w_mag_db":       args.w_mag_db,
            "db_loss":        "weighted dB RSE",
            "db_weight_alpha": args.db_weight_alpha,
            "db_weight_max":  args.db_weight_max,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train BigResUNet v1.1 — v1 + weighted dB RSE")
    parser.add_argument("--project-root",  type=str,   default=".")
    parser.add_argument("--output-dir",    type=str,   default="NNModel")
    parser.add_argument("--model-name",    type=str,   default="trained_model_resunet_v1_1.pt")
    parser.add_argument("--scaler-name",   type=str,   default="scaler_resunet_v1_1.gz")
    parser.add_argument("--history-name",  type=str,   default="history_resunet_v1_1.csv")
    parser.add_argument("--meta-name",     type=str,   default="meta_resunet_v1_1.json")
    ...
    parser.add_argument("--db-weight-alpha", type=float, default=1.0)   # NEW
    parser.add_argument("--db-weight-max",   type=float, default=10.0)  # NEW
