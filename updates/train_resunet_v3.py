...
Problem diagnosed in Big v2:
  - old_excel RSE: Big v2 = 0.015 (beats Antenna NN!)
  - LHS test RSE:  Big v2 = 0.027 (2x worse than Antenna NN)
  This is distribution overfit: model memorised old_excel geometry distribution
  and fails to generalise to LHS geometry distribution.

Fixes applied vs Big v2:
  1. Combined dataset: old_excel + ALL LHS seeds mixed together, 80/20 split
     across combined data → val loss now measures LHS generalisation directly.
  2. Dropout(0.1) in cond_mlp → prevents geometry encoder from overfitting.
  3. weight_decay: 1e-4 → 5e-4 → stronger L2 regularisation.
  4. Epochs: 300 → 250 (best-val checkpoint still saved).
...
COMPARISON TABLE:
  Model       dataset        dropout  weight_decay  epochs
  Big v2      old_excel only  NO      1e-4          300
  Big v3      old_excel+LHS   YES     5e-4          250   ← this file
"""
...
def train(args: argparse.Namespace) -> None:
    ...
    meta = {
        ...
        "motivation": (
            "Big v2 (SmoothResUNet1D) overfitted to old_excel distribution (RSE 0.015) "
            "but failed on LHS test set (RSE 0.027 vs Antenna NN 0.015). "
            "Fix: combined dataset training + Dropout + higher weight_decay."
        ),
        "loss_weights": {
            "w_ri":        args.w_ri,
            "w_mag":       args.w_mag,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset",       type=str,   default="combined", choices=["combined", "old_excel", "lhs"])
