"""
train_resunet_big_v2.py  —  SmoothResUNet1D  (Big v2)

Motivation:
  Analysis showed ResUNets beat Antenna NN on dB RSE (by metric we optimised),
  but Antenna NN wins on magnitude RSE (the physically correct metric).
  Two structural reasons Antenna NN is robust to outliers:
    1. Fixed Gaussian smooth as final layer → can't produce catastrophically
       wrong sharp predictions. When wrong, it's "a little wrong" not "totally wrong".
    2. Trained with MSE(ri) which implicitly aligns with magnitude RSE.

  This model incorporates both insights:
    1. SmoothResUNet1D = BigResUNet1D + fixed Gaussian smooth output layer (σ=1.527)
    2. w_mag loss uses magnitude RSE instead of dB RSE

  σ=1.527 was determined by an initial training run with learnable sigma;
  the model converged to this value and it is now fixed.

COMPARISON TABLE:
  Model             base_ch  smooth_layer   mag_loss      epochs
  Antenna NN        —        YES (fixed σ)  MSE(ri)       —
  BigResUNet        128      NO             dB RSE        300
  SmoothResUNet     128      YES (fixed σ)  magnitude RSE 300  ← this file
"""
...
def train(args: argparse.Namespace) -> None:
    ...
    meta = {
        ...
        "motivation": (
            "BigResUNet1D + fixed Gaussian smooth output layer (sigma=1.527) + magnitude RSE loss. "
            "Incorporates two structural insights from Antenna NN: (1) output smoothing "
            "for outlier robustness, (2) magnitude-aligned training objective."
        ),
        "loss_weights": {
            "w_ri":        args.w_ri,
            "w_mag":       args.w_mag,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SmoothResUNet1D — BigResUNet + smooth layer + mag RSE loss")
    ...
    parser.add_argument("--model-name",    type=str,   default="trained_model_resunet_smooth.pt")
    parser.add_argument("--scaler-name",   type=str,   default="scaler_resunet_smooth.gz")
    parser.add_argument("--history-name",  type=str,   default="history_resunet_smooth.csv")
    parser.add_argument("--meta-name",     type=str,   default="meta_resunet_smooth.json")
