"""
train_resunet_big_v4.py  —  BigResUNet v4 (= Big v2 + early stopping)

Identical to Big v2 (SmoothResUNet1D) in every way except:
  - Early stopping with patience=40 → stops when val loss plateaus
  - Max epochs set to 500 (but early stopping fires sooner)

COMPARISON TABLE:
  Model   base_ch  smooth_layer   mag_loss      early_stop  epochs
  Big v2  128      YES (σ=1.527)  magnitude RSE NO          300
  Big v4  128      YES (σ=1.527)  magnitude RSE YES(p=40)   ≤500  ← this file
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train BigResUNet v4 — Big v2 + early stopping")
    ...
    parser.add_argument("--model-name",    type=str,   default="trained_model_resunet_v4.pt")
    parser.add_argument("--scaler-name",   type=str,   default="scaler_resunet_v4.gz")
    parser.add_argument("--history-name",  type=str,   default="history_resunet_v4.csv")
    parser.add_argument("--meta-name",     type=str,   default="meta_resunet_v4.json")
