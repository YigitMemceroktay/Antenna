...
class DualResUNet1D(nn.Module):
    def __init__(self, input_dim: int = 11, target_len: int = 201, base_ch: int = 64) -> None:
        super().__init__()
        self.target_len = target_len
        self.cond_dim = base_ch

        self.cond_mlp = nn.Sequential(
            nn.Linear(input_dim, 128),
            nn.GELU(),
            nn.Linear(128, self.cond_dim),
            nn.GELU(),
        )

        self.pos_channels = 16
        self.enc1 = ResBlock1D(self.cond_dim + self.pos_channels, base_ch)
    ...
    def positional_encoding(self, length: int, device: torch.device) -> torch.Tensor:
        ...
        out = torch.cat(feats[: self.pos_channels], dim=1)
        return out

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b = x.shape[0]
        cond = self.cond_mlp(x).unsqueeze(-1).repeat(1, 1, self.target_len)
        pe = self.positional_encoding(self.target_len, x.device).repeat(b, 1, 1)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train dual-output ResUNet for antenna S-parameter prediction")
    ...
    parser.add_argument("--model-name", type=str, default="trained_model_resunet_dual.pt")
    parser.add_argument("--scaler-name", type=str, default="scaler_resunet_dual.gz")
    parser.add_argument("--history-name", type=str, default="history_resunet_dual.csv")
    parser.add_argument("--meta-name", type=str, default="meta_resunet_dual.json")
