"""
train_resunet_big_v8.py  —  BigResUNet v8 (= v1.2 + max-deviation penalty)

v1.2 = v1 + w_mag_db raised from 1.0 → 3.0.
       BigResUNet1D, no smooth layer, CosineAnnealingLR.
       NewData RSE-dB mean: 0.01213, p100: 0.103  (second-best after v1).

v8 adds one extra term on top of v1.2's 5-term loss:
  w_max_dev * max_deviation_loss

max_deviation_loss = mean_over_batch(
    max_over_freq( |mag_p - mag_t| / (mag_t + eps) )
)
Forces the model to care about its single worst frequency error per sample,
targeting the tail of the error distribution.

Loss terms (6 total):
  w_ri         * MSE(real/imag)          ← from v1.2
  w_mag_db     * dB_RSE  (weight=3.0)   ← from v1.2
  w_slope      * MSE(dB slope)           ← from v1.2
  w_curv       * MSE(dB curvature)       ← from v1.2
  w_passivity  * passivity penalty       ← from v1.2
  w_max_dev    * max_deviation_loss      ← NEW

Architecture: BigResUNet1D (no smooth layer) — same as v1 / v1.2.
Hyperparameters: same as v1.2 (lr=8e-4, batch=32, wd=1e-4).
Training data: old_excel only — NewData / LHS are test sets (no leakage).
Early stopping: patience=40 (added vs v1.2).
"""
...
# ---------------------------------------------------------------------------
# Building blocks (identical to v1 / v1.2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Model — BigResUNet1D, no smooth layer (identical to v1 / v1.2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Loss  (6 terms — v1.2's 5 terms + max-deviation penalty)
# ---------------------------------------------------------------------------
...
def compute_loss(
    ...
    # Max-deviation penalty (NEW): worst single-frequency relative error per sample
    max_dev_loss = torch.mean(
        torch.amax(torch.abs(mag_p - mag_t) / (mag_t + mag_eps), dim=-1)
    )
...
def train(args: argparse.Namespace) -> None:
    ...
    print(f"BigResUNet1D (v8)  |  base_ch={args.base_channels}  |  trainable params: {n_params:,}")
    print(f"Device: {device}")
    print(f"w_mag_db={args.w_mag_db}  w_max_dev={args.w_max_dev}")
    ...
    meta = {
        ...
        "motivation": (
            "Big v8: v1.2 (BigResUNet1D + w_mag_db=3.0, 2nd best on NewData) "
            "+ max-deviation penalty (w_max_dev=0.5). "
            "Penalises the single worst frequency-point error per sample."
        ),
        ...
        "loss_weights": {
            ...
            "w_passivity":   args.w_passivity,
            "w_max_dev":     args.w_max_dev,
            "max_dev_loss":  "mean( max_freq( |mag_p - mag_t| / (mag_t + eps) ) )",
        },
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train BigResUNet v8 — v1.2 + max-deviation penalty")
    parser.add_argument("--project-root",  type=str,   default=".")
    parser.add_argument("--output-dir",    type=str,   default="NNModel")
    parser.add_argument("--model-name",    type=str,   default="trained_model_resunet_v8.pt")
    parser.add_argument("--scaler-name",   type=str,   default="scaler_resunet_v8.gz")
    parser.add_argument("--history-name",  type=str,   default="history_resunet_v8.csv")
    parser.add_argument("--meta-name",     type=str,   default="meta_resunet_v8.json")
    ...
    # Loss weights: v1.2 values + new w_max_dev
    parser.add_argument("--w-ri",          type=float, default=0.20)
    parser.add_argument("--w-mag-db",      type=float, default=3.00)   # same as v1.2
    ...
    parser.add_argument("--w-max-dev",     type=float, default=0.50)   # NEW
